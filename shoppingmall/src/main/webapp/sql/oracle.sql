drop table money_tbl_02;    /*참조하는 테이블을 먼저 드랍*/
drop table  member_tbl_02; /* 참조당하는 테이블은 나중에 드랍가능*/
...
select mb.custno, mb.custname, 		
		  case when mb.grade = 'A' then 'VIP'  when  grade = 'B' then '일반' else '직원' end AS  grade,
		  sum(mo.price) 
...
select  custno, custname, phone, address, joindate, grade, city from member_tbl_02 where custno=100007; 

update member_tbl_02 set custname ='홍길동', phone ='010-2222-1234', address='서울시 송파구', joindate='20230921', grade='A', city='05' 
where custno = 100007;
